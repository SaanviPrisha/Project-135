...
solar_mass = dataframe["solar_mass"].tolist()
solar_radius = dataframe["solar_radius"].tolist()
...
for data in solar_mass:
    si_unit = float(data)*1.989e+30
    solar_mass_si_unit.append(si_unit)

for data in solar_radius:
    si_unit = float(data)* 6.957e+8
    solar_mass_si_unit.append(si_unit)

star_masses = solar_mass_si_unit
star_radiuses = solar_mass_si_unit
...
star_distance_list = dataframe["distance"].tolist()
star_distance_list.pop(0)

print(star_distance_list)

star_gravity_list = dataframe["star_gravity"].tolist()
...
for data in star_distance_list:
    if data > 100:
        dataframe.remove(data)
    new_data.append(dataframe)
...
for data in star_gravity_list:
    if data > 350 or data < 150:
        dataframe.remove(data)
    new_data2.append(dataframe)
star_names = dataframe["star_names"].tolist()
star_names.pop(0)

star_mass = dataframe["mass"].tolist()
star_mass.pop(0)

star_radius = dataframe["radius"].tolist()
star_radius.pop(0)

star_distance = dataframe["distance"].tolist()
star_distance.pop(0)

star_gravity = dataframe["star_gravity"].tolist()
star_gravity.pop(0)
plt.figure()
